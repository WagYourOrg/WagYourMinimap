...
public class ChunkLoadStrategy extends AbstractChunkUpdateStrategy {
    ...
    public synchronized ChunkData loadFromChunk(ChunkAccess chunk, Level level, MapRegion parent) {
        ...
        for (int i = 0; i < 256; ++i) {
            ...
            data.blockid[i] = data.getOrRegisterResourceLocation(Registry.BLOCK.getKey(chunk.getBlockState(blockPos.set(pos.x << 4 + x, data.heightmap[i], pos.z << 4)).getBlock()));
    ...
    protected void registerEventListener() {
        ChunkEvent.LOAD_DATA.register((ChunkAccess chunk, @Nullable ServerLevel level, CompoundTag nbt) -> {
            String server_slug = INSTANCE.getServerName();
            String level_slug = INSTANCE.getLevelName(level);
            ChunkPos pos = chunk.getPos();
            int index = MapRegion.chunkPosToIndex(pos);
            MapLevel.Pos region_pos = new MapLevel.Pos(pos.getRegionX(), pos.getRegionZ());
            updateChunk(server_slug, level_slug, region_pos, index, (region, oldData) ->
                loadFromChunk(chunk, INSTANCE.resolveServerLevel(level), region)
            );
        });
    }
